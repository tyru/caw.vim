...
" From GetVimIndent() in runtime/indent/vim.vim
function! s:get_vim_indent_num(lnum) "{{{
  ...
  let ind = indent(lnum)
  if getline(a:lnum) =~ '^\s*\\' && a:lnum > 1 && getline(lnum) !~ '^\s*\\'
  ...
  if getline(a:lnum) =~ '^\s*\(ene\@!\|cat\|fina\|el\|aug\%[roup]\s*!\=\s\+END\)'
    let ind = ind - &sw
  endif
...
function! s:get_indent_num(lnum) "{{{
    ...
    elseif &syntax ==# 'vim'
        return s:get_vim_indent_num(a:lnum)
    else
